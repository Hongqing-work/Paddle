...
class ScheduleModule {
 public:
  ...
  explicit ScheduleModule(const std::vector<stmt::BlockRef>& blocks)
      : blocks_(blocks) {}
  explicit ScheduleModule(std::vector<stmt::BlockRef>&& blocks)
      : blocks_(std::move(blocks)) {}

  //! Get all the Expr in this ScheduleModule.
  std::vector<stmt::BlockRef> GetBlocks() { return blocks_; }

  std::vector<stmt::BlockRef> GetBlocks() const { return blocks_; }

  void SetBlocks(const std::vector<stmt::BlockRef>& blocks) {
    blocks_ = blocks;
  }
 ...
 private:
  ...
  //! computation kernel.
  std::vector<stmt::BlockRef> blocks_;
};
...
class ScheduleBase {
 public:
  ...
  virtual std::vector<stmt::For> GetLoops(
      const stmt::Schedule& target_sched) const = 0;
  virtual std::vector<stmt::For> GetLoops(
      const std::string& sched_name) const = 0;
  virtual std::vector<stmt::Schedule> GetAllSchedStmts() const = 0;
  virtual std::vector<stmt::Schedule> GetChildSchedStmts(
      const stmt::StmtRef& stmt) const = 0;
  virtual stmt::Schedule GetSchedStmt(const std::string& sched_name) const = 0;

  virtual std::vector<stmt::For> Split(const stmt::For& loop,
 ...
 protected:
  void Replace(const stmt::StmtRef& src_stmt, const stmt::StmtRef& tgt_stmt);
