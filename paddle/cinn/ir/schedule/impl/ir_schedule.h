...
class DyScheduleImpl : public ScheduleBase {
 public:
  ...
  std::vector<stmt::For> GetLoops(const stmt::Schedule& target_sched) const;
  std::vector<stmt::For> GetLoops(const std::string& target_sched_name) const;
  std::vector<stmt::Schedule> GetAllSchedStmts() const;
  std::vector<stmt::Schedule> GetChildSchedStmts(
      const stmt::StmtRef& stmt) const;
  stmt::Schedule GetSchedStmt(const std::string& sched_name) const;
  std::vector<stmt::For> Split(const stmt::For& loop,
